defmodule Klepsidra.Repo.Migrations.CreateUnifiedSearchView do
  ...
  def up do
    ...
    SELECT bpn.rowid + 1_000_000_000_000 AS rowid,
    bpn.id AS entity_id,
    ...
    ON bpn.business_partner_id = bp.id
    UNION ALL
    -- Business partners
    SELECT rowid + 2_000_000_000_000 AS rowid,
    id AS entity_id, 'business_partner' AS entity,
    CASE
    ...
    ON di.country_id = lc.iso_3_country_code
    UNION ALL
    -- Document types
    SELECT
    ...
    UNION ALL
    -- Journal entries
    SELECT je.rowid + 5_000_000_000_000 AS rowid,
    je.id AS entity_id, 'journal_entry' AS entity, jety.name AS category, 
    NULL AS status, je.user_id AS owner_id, NULL AS group_id,
    ...
    ON je.entry_type_id = jety.id
    LEFT JOIN journal_entry_tags jet
    ON je.id = jet.journal_entry_id
    LEFT JOIN tags t
    ON jet.tag_id = t.id
    LEFT JOIN locations_cities lc
    WHERE je.is_private = 0
    UNION ALL
    -- Journal entry types
    SELECT
    ...
    SELECT kmn.rowid + 7_000_000_000_000 AS rowid,
    kmn.id AS entity_id,
    ...
    SELECT rowid + 8_000_000_000_000 AS rowid,
    id AS entity_id,
    ...
    UNION ALL
    -- Project notes
    SELECT pn.rowid + 9_000_000_000_000 AS rowid,
    pn.id AS entity_id,
    ...
    UNION ALL
    -- Projects
    SELECT p.rowid + 10_000_000_000_000 AS rowid,
    p.id AS entity_id, 'project' AS entity, 'Project' AS category,
    CASE
    WHEN p.active THEN 'Active'
    ELSE 'Inactive'
    END AS status,
    NULL AS owner_id, NULL AS group_id,
    p.name AS title, NULL AS subtitle,
    GROUP_CONCAT(t.name, ' · ') AS tags,
    NULL AS location,
    ...
    ON pt.tag_id = t.id
    LEFT JOIN business_partners bp
    ON p.business_partner_id = bp.id
    UNION ALL
    -- Timer notes
    SELECT tn.rowid + 11_000_000_000_000 AS rowid,
    ...
    UNION ALL
    -- Timers
    SELECT t.rowid + 12_000_000_000_000 AS rowid,
    t.id AS entity_id, 'timer' AS entity, 'Timer' AS category,
    CASE
    ...
    ON tt.tag_id = ta.id
    LEFT JOIN business_partners bp
    ON T.business_partner_id = bp.id
    LEFT JOIN projects p
    ...
    UNION ALL
    -- Trips
    SELECT tt.rowid + 13_000_000_000_000 AS rowid,
    tt.id AS entity_id, 'travel_trip' AS entity, 'Trip' AS category,
    CASE
    ...
    ON tt.user_id = u.id
    LEFT JOIN locations_countries lc
    ON tt.country_id = lc.iso_3_country_code
    UNION ALL
    -- User documents
    SELECT
    ud.rowid + 14_000_000_000_000 AS rowid, ud.id AS entity_id,
    'user_document' AS entity, 'User document' AS category,
    CASE
    ...
    ON ud.user_id = u.id
    LEFT JOIN locations_countries lc
    ON ud.country_id = lc.iso_3_country_code
    );
    """)
  end
